...
class array:
  ...
  def __setitem__(self, mask, val):
    ...
    arr[int, int] = val
    arr[mask] = scalar or another array
    '''
    val_pointer = 0
    if isinstance(mask, (list, array)) and len(mask)>0:
      ...
      self.arr[mask] = val
    elif isinstance(mask, tuple) and isinstance(mask[0], int) and isinstance(mask[1], int):
      r, c = self.shape
      sr, sc = self.strides
      mr, mc = mask
      if mr<0:
